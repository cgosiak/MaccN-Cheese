/*	____________________________________________________________________________
	    ...
	    Semantics and Coding Component Implementation for the MicroA Compiler
...
int CodeGen::strSize(const string fsize)
{
	int size = fsize.length();
	if (size % 2 == 0)
	{
		return (size + 2);
	}
...
void CodeGen::Finish()
{
	...
	// label for the strings
	Generate("LABEL     ", "STRINGS", "");
	
	// if the string table isn't empty make the strings (assembly)
...
// code that goes through strings in buffer preping them for assembly
void CodeGen::WriteString()
{
	...
	// recreates string adding in assembly escape character to show quotes and colons
	...
	// puts string in string table
	stringTable.push_back(newString);
	// assembly to write string
	Generate("WRST       ", "+" + to_string(strSum) + "(R14)", "");
	strSum += strSize(s);
}
